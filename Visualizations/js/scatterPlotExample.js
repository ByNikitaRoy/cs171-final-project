...
function drawFinalScatterplot() {
    ...
    var mobileScreen = document.getElementById("chart-wrapper-final").offsetWidth < 550 ? true : false;
...
function drawCellCircleScatterplot() {
    ...
    //Quick fix for resizing some things for mobile-ish viewers
    var mobileScreen = ( document.getElementById("chart-wrapper-cell-circle").offsetWidth < 550 ? true : false);

    //Scatterplot
    var margin = {left: 30, top: 20, right: 20, bottom: 30},
        width = Math.min( document.getElementById("chart-cell-circle").offsetWidth, 800) - margin.left - margin.right,
        height = width*2/3;

    var svg = d3.select("#chart-cell-circle").append("svg")
        .attr("width", (width + margin.left + margin.right))
        .attr("height", (height + margin.top + margin.bottom));
    ...
    if (!mobileScreen) {
        //Draw the legend
        drawLegend("chart-cell-circle","legend-cell-circle", opacityCircles, color, rScale);
    } else {
    ...
    function removeTooltipCellCircle (d, i) {
        ...
        //Save the chosen circle (so not the voronoi)
        var element = d3.selectAll("#chart-cell-circle .countries."+d.CountryCode);
...
function drawSimpleScatterplot() {
    ...
    //Quick fix for resizing some things for mobile-ish viewers
    var mobileScreen = document.getElementById("chart-wrapper-simple").offsetWidth < 550 ? true : false;

    //Scatterplot
    var margin = {left: 30, top: 20, right: 20, bottom: 30},
        width = Math.min( document.getElementById("chart-simple").offsetWidth, 800) - margin.left - margin.right,
        height = width*2/3;

    var svg = d3.select("#chart-simple").append("svg")
        .attr("width", (width + margin.left + margin.right))
        .attr("height", (height + margin.top + margin.bottom));
