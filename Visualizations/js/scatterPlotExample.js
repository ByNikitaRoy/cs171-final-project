// Used in the blog "2015-07-20-voronoi" //
...
function drawFinalScatterplot() {
  ...
  var mobileScreen =
    document.getElementById("chart-wrapper-final").offsetWidth < 550
      ? true
      : false;
  ...
  var margin = { left: 30, top: 20, right: 20, bottom: 30 },
    width =
      Math.min(document.getElementById("chart-final").offsetWidth, 800) -
      margin.left -
      margin.right,
    height = (width * 2) / 3;
  ...
  var xScale = d3.scale.log().range([0, width]).domain([100, 2e5]); //I prefer this exact scale over the true range and then using "nice"
  ...
  var yScale = d3.scale
    ...
    .range([height, 0])
    .domain(
      d3.extent(countries, function (d) {
...
////////////////////////////////////////////////////////////
////////////////// Show cells and circles //////////////////
////////////////////////////////////////////////////////////

function drawCellCircleScatterplot() {
  //Quick fix for resizing some things for mobile-ish viewers
  var mobileScreen =
    document.getElementById("chart-wrapper-cell-circle").offsetWidth < 550
      ? true
      : false;
  ...
  var margin = { left: 30, top: 20, right: 20, bottom: 30 },
    width =
      Math.min(document.getElementById("chart-cell-circle").offsetWidth, 800) -
      margin.left -
      margin.right,
    height = (width * 2) / 3;

  var svg = d3
    .select("#chart-cell-circle")
    .append("svg")
    .attr("width", width + margin.left + margin.right)
    .attr("height", height + margin.top + margin.bottom);
  ...
  var circlesOuter = circleClipGroup
    ...
    .attr("r", maxDistanceFromPoint)
    .on("mouseover", showTooltipCellCircle)
    .on("mouseout", removeTooltipCellCircle);
  ...
  var voronoiGroup = wrapper.append("g").attr("class", "voronoiWrapper");
  ...
  if (!mobileScreen) {
    ...
    d3.select("#legend-cell-circle-wrapper").style("display", "none");
  }
  ...
  //Hide the tooltip when the mouse moves away
  function removeTooltipCellCircle(d, i) {
    //Save the chosen circle (so not the voronoi)
    var element = d3.selectAll(
      "#chart-cell-circle .countries." + d.CountryCode
    );
    ...
    //Fade out guide lines, then remove them
    d3.selectAll("#chart-cell-circle .guide")
      .transition()
      .duration(200)
      .style("opacity", 0)
      .remove();
  ...
  //Show the tooltip on the hovered over slice
  function showTooltipCellCircle(d, i) {
    //Save the chosen circle (so not the voronoi)
    var element = d3.selectAll(
      "#chart-cell-circle .countries." + d.CountryCode
    );
...
drawCellCircleScatterplot();

////////////////////////////////////////////////////////////
/////////////////// Tooltip to circle //////////////////////
////////////////////////////////////////////////////////////

function drawSimpleScatterplot() {
  //Quick fix for resizing some things for mobile-ish viewers
  var mobileScreen =
    document.getElementById("chart-wrapper-simple").offsetWidth < 550
      ? true
      : false;
  ...
  var margin = { left: 30, top: 20, right: 20, bottom: 30 },
    width =
      Math.min(document.getElementById("chart-simple").offsetWidth, 800) -
      margin.left -
      margin.right,
    height = (width * 2) / 3;
  ...
  //.nice();
  //Set new x-axis
  var xAxis = d3.svg.axis().orient("bottom").ticks(5).scale(xScale);
  //Append the x-axis
  wrapper
    .append("g")
    .attr("class", "x axis")
  ...
  var yScale = d3.scale
    ...
    .nice();
  var yAxis = d3.svg.axis().orient("left").ticks(6).scale(yScale);
  //Append the y-axis
  wrapper
    .append("g")
    .attr("class", "y axis")
  ...
  wrapper
    ...
    .on("mouseover", showTooltipSimple)
    .on("mouseout", removeTooltipSimple);
  ...
  if (!mobileScreen) {
    //Draw the legend
    drawLegend("chart-simple", "legend-simple", opacityCircles, color, rScale);
  } else {
    d3.select("#legend-simple-wrapper").style("display", "none");
  }
  ...
  //Show the tooltip on the hovered over slice
  function showTooltipSimple(d) {
    //Define and show the tooltip
    jQuery(this).popover({
      placement: "auto top",
      container: "#chart-simple",
      trigger: "manual",
      html: true,
      content: function () {
        return (
...
function drawLegend(svgName, svgLegendName, opacityCircles, color, rScale) {
  ...
  var bubbleSizeLegend = legendWrapper
    .append("g")
    .attr(
      "transform",
      "translate(" +
        (legendWidth / 2 - 30) +
        "," +
        (color.domain().length * rowHeight + 20) +
        ")"
    );
