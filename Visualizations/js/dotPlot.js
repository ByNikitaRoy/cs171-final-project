class DotPlot {
  ...
  initVis() {
    ...
    vis.yScale = d3
      ...
      .range([
        vis.margin.bottom,
        vis.height - vis.margin.bottom - vis.margin.top,
      ])
      .domain([15000000, 400]);

    vis.rScale = d3.scaleLog().range([40, 0]).domain([15000000, 400]);
    ...
    vis.xAxisGroup
      .call(d3.axisBottom().scale(vis.xScale).ticks(0))
      .attr("class", "xAxis");
    ...
    vis.svg
      ...
      .attr(
        "transform",
        `translate (${vis.width - vis.width * (1 / 6)}, ${vis.margin.top})`
      );
    ...
    vis.svg
      ...
      .attr("viewBox", [
        0,
        0,
        vis.width - vis.padding * 2,
        vis.height - vis.padding * 2,
      ]);
  ...
  wrangleData() {
    ...
    this.selectedTime =
      this.dataByCountryInit[0].days[this.timeIndexSelected].date;
    ...
    //group by country
    vis.dataByCountry = Array.from(
      d3.group(vis.filteredData, (d) => d.Country),
      ([country, days, sum, tone, continent]) => ({
        country,
        days,
        sum,
        tone,
  ...
  updateVis() {
    let vis = this;
    // Add dots
    let dots = vis.svg.selectAll("circle").data(vis.dataByCountry);
    ...
    dots
      ...
      .on("mouseover", function (event, d) {
        d3.select(this).attr("stroke-width", "10px").attr("stroke", "#FFFFFF");
        //.attr('fill', '#FFFFFF');
  ...
  initWrangleData() {
    ...
    //group by country
    vis.dataByCountryInit = Array.from(
      d3.group(vis.filteredData, (d) => d.Country),
      ([country, days, sum, tone, continent]) => ({
        country,
        days,
  ...
  control() {
    ...
    dateLabelEnter.attr("x",-30).attr("y", 0);
